...
class Matrice:
 ...
 #Initialise la matrice avec les scores de gap sur la premier ligne et premiere colonne (le reste a 0)
 def mtInit(self) : 
  ...
  for i in range(lengthAli1) :
   ...
   for j in range(lengthAli2) :
    self.mt[i].append[self.scoreGa()*j]
    if i<=1 : 
     self.__mt[i].append([self.scoreGa()*i])
     if j<=1 : 
 ...
 def scoreUp(self,i,j) :
  return self.__mt[i-1][j]+self.scoreGa()

 #Calcul avec la case de gauche 
 def scoreLeft(self,i,j) : 
  return self.__mt[i][j-1]+self.scoreGa()

 #Calcul avec la case diagonale                    
 def scoreDiag(self,i,j) :
  if testMatch() :  
   return mt[i-1][j-1] + self.scoreMa(i-1,j-1)
  else :  
   return mt[i-1][j-1] + self.scoreGa(i-1,j-1)
 ...
 def mtFilling(self,s1,s2) :
  ...
  for i in range(lengthAli1) : 
   for j in range(lengthAli2) : 
   	resBest = bestScore(self.scoreDiag(mt[i][j]),self.scoreLeft(mt[i][j]),self.scoreUp(mt[i][j]))
   	print(resBest)
